...
public class OrderServiceImpl implements OrderService {
	...
	public void placeOrder(Cart cart) {
		...
		order.setProduct(getListOfProducts(cart.getItems()));
		order.setQuantity(order.getProduct().size());
	...
	// Helper
	// Methods--------------------------------------------------------------------------
	List<Product> getListOfProducts(List<Items> items) {
		List<Product> products = items.stream().map(item -> new Product(item.getProductId(), item.getProductName() , item.getImage(),item.getPrice(),item.getQuantity() ))
